...
class RayTracer {
    ...
    private getNaturalColor(thing: Thing, pos: Vector, norm: Vector, rd: Vector, scene: Scene) {
        let addLight = (col: any, light: any) => {
    ...
    render(scene: any, ctx: any, screenWidth: any, screenHeight: any) {
        let getPoint = (x: any, y: any, camera: any) => {
            let recenterX = (x: any) => (x - (screenWidth / 2.0)) / 2.0 / screenWidth;
            let recenterY = (y: any) => -(y - (screenHeight / 2.0)) / 2.0 / screenHeight;
